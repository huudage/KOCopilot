...
function bindFeatureOneUploader() {
  const fileInput = document.getElementById("koc-asr-file");
  if (!fileInput) return;
  const status = document.getElementById("koc-asr-status");
  ...
  fileInput.addEventListener("change", async (ev) => {
    ...
    try {
      ...
      if (window.KOCApi && typeof window.KOCApi.showToast === "function") {
        window.KOCApi.showToast("ASR 识别完成", "success");
      }
    } catch (e) {
      console.error(e);
      ...
      if (window.KOCApi && typeof window.KOCApi.showToast === "function") {
        window.KOCApi.showToast(msg, "error");
      }
    } finally {
