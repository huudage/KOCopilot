/**
 * KOCopilot — Persona Editor (v0.10).
 *
 * 一个全局单例的 Modal 编辑器，负责把一个 persona 对象渲染为表单、收集校验后的
 * 更新并通过 onSave 回调返回。它**不直接持久化**——存写动作交给 KOCHistory 或
...
(function () {
  ...
  function ensureModal() {
    ...
    modal.className = "koc-modal koc-persona-editor-modal";
    modal.setAttribute("hidden", "");
    modal.setAttribute("role", "dialog");
    modal.setAttribute("aria-modal", "true");
    modal.setAttribute("aria-labelledby", "koc-persona-editor-title");
    modal.innerHTML =
      '<div class="koc-modal__backdrop" data-koc-editor-action="close"></div>' +
      '<div class="koc-modal__panel">' +
        '<div class="koc-modal__head">' +
          '<h3 id="koc-persona-editor-title">编辑人设方案</h3>' +
          '<button class="koc-modal__close" type="button" data-koc-editor-action="close" aria-label="关闭">×</button>' +
        "</div>" +
        '<div class="koc-modal__body koc-persona-editor__body" data-koc-editor-body></div>' +
        '<div class="koc-modal__foot">' +
          '<span class="koc-persona-editor__hint">编辑后立即覆盖本地存档，不可撤销。</span>' +
          '<div style="display:flex; gap:0.5rem;">' +
            '<button type="button" class="btn btn-ghost sm" data-koc-editor-action="cancel">取消</button>' +
            '<button type="button" class="btn btn-primary sm" data-koc-editor-action="save">保存修改</button>' +
          "</div>" +
        "</div>" +
      "</div>";
    document.body.appendChild(modal);

    titleEl = modal.querySelector("#koc-persona-editor-title");
    bodyEl = modal.querySelector("[data-koc-editor-body]");
    saveBtn = modal.querySelector('[data-koc-editor-action="save"]');
    cancelBtn = modal.querySelector('[data-koc-editor-action="cancel"]');
    closeBtn = modal.querySelector('.koc-modal__close[data-koc-editor-action="close"]');
    backdrop = modal.querySelector('.koc-modal__backdrop[data-koc-editor-action="close"]');
  ...
  function renderField(def, value) {
    var id = "koc-pe-" + def.key;
    var hintHtml = def.hint
      ? '<div class="koc-persona-editor__hint">' + escapeAttr(def.hint) + "</div>"
      : "";
    var headHtml =
      '<div class="koc-persona-editor__field-head">' +
        '<label for="' + id + '">' + escapeAttr(def.label) +
        (def.required ? ' <span class="koc-persona-editor__req">*</span>' : "") +
        "</label>" +
        (typeof def.maxlen === "number"
          ? '<span class="koc-persona-editor__counter" data-counter-for="' + id + '">0/' + def.maxlen + "</span>"
          : "") +
      "</div>";
    ...
    if (def.type === "textarea") {
      var rows = def.rows || 3;
      return (
        '<div class="koc-persona-editor__field" data-field-key="' + def.key + '">' +
          headHtml +
    ...
    if (def.type === "score") {
      ...
      for (var s = 1; s <= 5; s++) {
        var on = s <= score;
        stars +=
          '<button type="button" class="koc-persona-editor__star' + (on ? " is-on" : "") +
      ...
      return (
        '<div class="koc-persona-editor__field" data-field-key="' + def.key + '">' +
          headHtml +
          '<div class="koc-persona-editor__stars" id="' + id + '" data-score-current="' + score + '">' +
            stars +
          "</div>" +
          hintHtml +
        "</div>"
  ...
  function bindFieldInteractivity() {
    bodyEl.querySelectorAll('input[type="text"], textarea').forEach(function (el) {
      ...
      if (!counter) return;
      var def = FIELD_DEFS.find(function (d) { return "koc-pe-" + d.key === el.id; });
    ...
    bodyEl.querySelectorAll(".koc-persona-editor__stars").forEach(function (group) {
      group.querySelectorAll(".koc-persona-editor__star").forEach(function (btn) {
        btn.addEventListener("click", function () {
          var v = parseInt(btn.dataset.scoreValue, 10);
          if (!Number.isFinite(v)) return;
          group.dataset.scoreCurrent = String(v);
          group.querySelectorAll(".koc-persona-editor__star").forEach(function (b) {
  ...
  function collect() {
    ...
    for (var i = 0; i < FIELD_DEFS.length; i++) {
      ...
      var id = "koc-pe-" + def.key;
      if (def.type === "score") {
        var group = bodyEl.querySelector('.koc-persona-editor__stars[id="' + id + '"]');
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (window.KOCApi && window.KOCApi.showToast) {
        KOCApi.showToast("保存失败：" + (e && e.message ? e.message : "未知错误"), "error");
      } else {
