...
<#
.SYNOPSIS
  Configure a separate GitHub account for KOCopilot so it never collides with
...
.DESCRIPTION
  ...
  After this script, your KOCopilot pushes will travel over the new key/account,
...
Write-Host ""
Write-Host "  Title 填: KOCopilot ($env:COMPUTERNAME)"
...
# ------------------------------------------------------------------
Section "5. 在 KOCopilot 本仓库设 local user.name / user.email"
...
Write-Host ""
Write-Host "    .\scripts\push-to-github.cmd $alias`:$Username/kocopilot.git" -ForegroundColor Green
Write-Host ""
