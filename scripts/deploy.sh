#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# KOCopilot — production deploy / upgrade script
#
...
#   4. systemctl restart kocopilot-server
#   5. Health check; on failure, roll back to the snapshot
#
# Run as the project user (e.g. `kocopilot`), NOT as root:
...
# ---- Pre-flight ----
log "===== KOCopilot deploy started ====="
cd "${PROJECT_DIR}" || die "PROJECT_DIR not found: ${PROJECT_DIR}"

if [[ "${EUID}" -eq 0 ]]; then
  die "do NOT run deploy.sh as root; switch to the project user (e.g. kocopilot)."
fi
