...
.PARAMETER CommitMessage
  Optional. Defaults to "feat: initial commit (KOCopilot v0.5)".

.EXAMPLE
  .\scripts\push-to-github.ps1 -RepoUrl https://github.com/yourname/kocopilot.git

.EXAMPLE
  .\scripts\push-to-github.ps1 `
      -RepoUrl git@github.com:yourname/kocopilot.git `
...
param(
  ...
  [string]$GitUserName,
  [string]$GitUserEmail,
  [string]$CommitMessage = "feat: initial commit (KOCopilot v0.5)"
)
...
# Refuse to run if cwd is not the KOCopilot repo root. This is critical because
...
foreach ($m in $markers) {
  if (-not (Test-Path $m)) {
    Die ("must be run from the KOCopilot project root (cwd=$PWD). Missing marker: $m`n" +
         "       Hint: cd into the project root first, e.g.`n" +
         "         cd D:\nocode\koc-copilot`n" +
         "         .\scripts\push-to-github.cmd <repo-url> [-CommitAuthor*]")
  }
}
OK "cwd is KOCopilot root: $PWD"
