...
<#
.SYNOPSIS
  Upload KOCopilot to the chronic-medication server (47.239.58.145) via tar + scp.
...
.DESCRIPTION
  ...
  Steps:
    ...
    3. SSH in and extract to /opt/kocopilot (creating the dir if needed)
...
.PARAMETER RemoteDir
  Where to put the code on the server. Default: /opt/kocopilot
...
param(
  ...
  [string]$RemoteDir = "/opt/kocopilot"
)

$ErrorActionPreference = "Stop"

$projectRoot = (Resolve-Path "$PSScriptRoot\..").Path
$tarballName = "kocopilot-upload-$(Get-Date -Format 'yyyyMMdd-HHmmss').tar.gz"
$tarballPath = Join-Path $env:TEMP $tarballName

Write-Host "==== KOCopilot upload ====" -ForegroundColor Cyan
...
Write-Host "  sudo DOMAIN=kocopilot.zlhu.asia bash $RemoteDir/scripts/install-on-medi-server.sh"
