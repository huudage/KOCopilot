#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# KOCopilot — install onto an existing chronic-medication-assistant server.
#
...
#   2. Creates the kocopilot system user + /opt/kocopilot directory
#   3. Clones (or pulls) the repo into /opt/kocopilot
#   4. Bootstraps server/venv and installs Python deps
#   5. Writes /opt/kocopilot/server/.env (interactive prompt for Keys)
#   6. Installs systemd unit `kocopilot-server` and starts it
...
# ---------------------------------------------------------------------------
PROJECT_DIR="${PROJECT_DIR:-/opt/kocopilot}"
RUN_USER="${RUN_USER:-kocopilot}"
BACKEND_PORT="${BACKEND_PORT:-5001}"
SERVICE_NAME="${SERVICE_NAME:-kocopilot-server}"
REPO_URL="${REPO_URL:-}"
BRANCH="${BRANCH:-main}"
DOMAIN="${DOMAIN:-}"

DEFAULT_DOMAIN="kocopilot.zlhu.asia"
if [[ -z "$DOMAIN" ]]; then
  read -rp "${C_INFO}KOCopilot 域名 [默认: ${DEFAULT_DOMAIN}]：${C_RESET} " DOMAIN
...
cp "$PROJECT_DIR/deploy/kocopilot-server.service" "$SERVICE_DST"
sed -i \
...
NGINX_AVAIL="/etc/nginx/sites-available/kocopilot.conf"
NGINX_LINK="/etc/nginx/sites-enabled/kocopilot.conf"
...
${C_OK}========================================================================${C_RESET}
${C_OK}  KOCopilot install scaffold complete${C_RESET}
${C_OK}========================================================================${C_RESET}
