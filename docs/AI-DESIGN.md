...
KOCopilot **每一个** LLM 调用都遵循同一个三层兜底范式：
...
| 用户中途刷新 | 前端 `KOCQAFlow.state` 重置；brief 表单恢复默认；骨架还在但需要重答（拆解便宜） |
...
| 字段 | 值 |
|---|---|
| **触发时机** | QA `done=true` 时由前端 `KOCQAFlow.generateScript()` 自动触发 |
...
| 脚本入库失败 | localStorage 配额满 → 静默丢弃（`koc-history.js` 的 `writeArray` catch 异常） |
...
| **前端 LLM 调用** | `interactions.js`（KOCQAFlow / bindSkeletonForm / bindPersonaForm / bindSeoForm / bindCommentsForm） |
| **前端 ASR 调用** | `asr-uploader.js`（ffmpeg.wasm 抽轨 + multipart 上传） |

---

> **更新规则**：每次给某个 AI 干预点改代码（无论是 prompt / router / schema），都要在本文对应章节同步更新；不允许出现"代码改了文档没改"的状态。本文是 KOCopilot AI 行为的**唯一权威说明**。
