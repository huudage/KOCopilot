...
> ⚠️ **待用户确认（不影响 KOCopilot 部署，但建议记录）**：
> - 服务器规格（vCPU / RAM / 带宽）
...
## 3. KOCopilot 域名规划（待执行）

| 项 | 计划值 | 备注 |
|---|---|---|
| 子域名 | `kocopilot.zlhu.asia` | 已 dig 确认未占用 |
...
| 项目路径 | `/opt/kocopilot` | 主项目预留 `/opt/medi` 或 `/var/www/chronic-medication` |
| 运行用户 | `kocopilot`（脚本自动创建） | 与主项目用户隔离 |
| systemd 单元 | `kocopilot-server.service` | 与主项目无冲突（主项目纯静态无 systemd） |

## 4. 不会影响主项目的 5 个理由（共存隔离）

| 维度 | 主项目（zlhu.asia） | KOCopilot（kocopilot.zlhu.asia） | 隔离强度 |
|---|---|---|---|
| nginx server 块 | 独立 server_name | 独立 server_name | ⭐⭐⭐⭐⭐ |
| TCP 端口 | 80 / 443（公网）| 80 / 443 共享 + 内部 5001 | ⭐⭐⭐⭐ |
| 文件路径 | 不动主项目 root | 独立 `/opt/kocopilot` | ⭐⭐⭐⭐⭐ |
| 系统用户 | 不动 | 独立 `kocopilot` 用户 | ⭐⭐⭐⭐⭐ |
| 进程 | 主项目纯静态无进程 | gunicorn + uvicorn workers | ⭐⭐⭐⭐⭐ |

回滚成本：删除 `/etc/nginx/sites-enabled/kocopilot.conf` + `nginx -s reload` + `systemctl stop/disable kocopilot-server` 即可彻底卸载，不影响主域名。
...
3. 点「添加记录」，填：
   ...
   - TTL：**600 秒**
4. 保存。等 5-10 分钟生效。
5. 本机验证：`Resolve-DnsName kocopilot.zlhu.asia` 应回 `47.239.58.145`
...
### 回滚（如果 KOCopilot 部署搞坏了主项目）

```bash
sudo systemctl stop kocopilot-server
sudo systemctl disable kocopilot-server
...
| 2026-05-04 | KOCopilot 后端从豆包标准版迁移到极速版（v0.5）；本地直调豆包真跑通，移除 ngrok / PUBLIC_BASE_URL / /asr-tmp/ 依赖 | curl POST 565KB wav → 2.83s 返回 transcript |
| (待填) | 添加 A 记录 `kocopilot.zlhu.asia` | 验证：`dig +short kocopilot.zlhu.asia` |
| (待填) | 火山控制台开通 `volc.bigasr.auc_turbo` 资源 | 验证：health-check.sh ASR 端点返回 200 |
| (待填) | 安装 KOCopilot（systemd + nginx） | 验证：`http://kocopilot.zlhu.asia/api/health` |
| (待填) | 申请 Let's Encrypt 证书 | 验证：`curl -I https://kocopilot.zlhu.asia` 无证书警告 |
