 ...
 *   the rest of the app keeps calling KOCHistory.* unchanged.
 *
 ...
 *   }]
 *   koc.skeletons.v1  → 旧版 schema（v0.6 之前），新代码不再写入；读侧也不展示，避免与
...
(function () {
  ...
  function renderPersonasBoard(ul) {
    ...
    ul.innerHTML = list
      ...
      .map(function (rec) {
        ...
        return (
          '<li class="koc-history-item" data-id="' + escapeHtml(rec.id) + '">' +
            '<div class="koc-history-item__row">' +
              "<b>" + title + "</b>" +
              "<span>" + (meta || "—") + "</span>" +
              '<span class="koc-pill">' + escapeHtml(pillTxt) + "</span>" +
              '<button type="button" class="btn btn-ghost sm koc-history-toggle" data-target="persona-detail-' + escapeHtml(rec.id) + '">展开 ↓</button>' +
              ' <button type="button" class="koc-history-del" data-kind="persona" data-id="' + escapeHtml(rec.id) + '" aria-label="删除">×</button>' +
            "</div>" +
            '<div class="koc-history-detail" id="persona-detail-' + escapeHtml(rec.id) + '" hidden>' + detailsHtml + "</div>" +
          "</li>"
        );
      })
      .join("");
    ...
    ul.innerHTML = list
      ...
      .map(function (rec) {
        ...
        var answersHtml = (rec.answers || [])
          .map(function (a) {
            return (
              '<div class="koc-history-detail__answer">' +
                "<b>第 " + a.round + " 题</b> · " + escapeHtml(a.question || "") +
                '<br/><span class="koc-history-detail__choice">✓ ' + escapeHtml(a.choice || "") + "</span>" +
              "</div>"
            );
          })
          .join("");
        ...
        if (rec.skeleton) {
          ...
          skeletonHtml =
            '<div class="koc-history-detail__sub">' +
        ...
        var fullScriptHtml = (rec.script && rec.script.full_text)
          ? '<div class="koc-history-detail__script"><pre>' + escapeHtml(rec.script.full_text) + "</pre></div>"
          : "";
        ...
        return (
          '<li class="koc-history-item" data-id="' + escapeHtml(rec.id) + '">' +
            '<div class="koc-history-item__row">' +
              ...
              '<span class="koc-pill">' + escapeHtml(pillTxt) + "</span>" +
              '<button type="button" class="btn btn-ghost sm koc-history-toggle" data-target="script-detail-' + escapeHtml(rec.id) + '">展开 ↓</button>' +
              ' <button type="button" class="koc-history-del" data-kind="script" data-id="' + escapeHtml(rec.id) + '" aria-label="删除">×</button>' +
            "</div>" +
            '<div class="koc-history-detail" id="script-detail-' + escapeHtml(rec.id) + '" hidden>' +
              (rec.persona
                ? '<div class="koc-history-detail__sub"><b>选用人设 · </b>' +
                    escapeHtml(rec.persona.name || "—") +
                    (rec.persona.differentiation ? "（" + escapeHtml(rec.persona.differentiation) + "）" : "") +
                  "</div>"
                : "") +
              skeletonHtml +
              (answersHtml ? '<div class="koc-history-detail__sub"><b>3 个创作决策</b></div>' + answersHtml : "") +
              fullScriptHtml +
              '<div class="koc-history-detail__actions">' +
  ...
  function bindToggleButtons() {
    document.querySelectorAll(".koc-history-toggle").forEach(function (btn) {
  ...
  // 后者通过 sessionStorage["koc.lastScriptForSeo"] 把脚本传递到 feature-3，
  ...
  function bindScriptActions() {
      ...
      btn.addEventListener("click", async function () {
        ...
        try {
          ...
          if (window.KOCApi && window.KOCApi.showToast) {
            KOCApi.showToast("已复制脚本（" + text.length + " 字）", "success");
          }
        } catch (e) {
    ...
    document.querySelectorAll('[data-action="bring-to-seo"]').forEach(function (btn) {
      ...
      btn.addEventListener("click", function () {
        ...
        try {
          sessionStorage.setItem("koc.lastScriptForSeo", rec.script.full_text);
        } catch (_) {}
