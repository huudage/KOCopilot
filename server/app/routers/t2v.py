"""Module 7 — Text-to-Video routing (v0.9, KOCopilot 第 7 个 AI 干预点).
...
router = APIRouter()
log = logging.getLogger("kocopilot.t2v_route")
...
def _resolve_user_id(request: Request, override: Optional[str]) -> str:
    ...
    if isinstance(trace_id, str) and len(trace_id) >= _USER_ID_MIN_LEN:
        ...
        return f"koc-{trace_id}"
    return f"koc-{uuid.uuid4().hex[:12]}"
